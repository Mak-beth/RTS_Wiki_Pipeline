...
The threaded pipeline uses `reqwest::blocking` on a dedicated OS thread. This means it cannot
share a Tokio runtime with the async pipeline. Throughput differences between the two pipelines
are a genuine architectural finding, not a defect.
